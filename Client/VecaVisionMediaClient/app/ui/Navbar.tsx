        ...
        <div>
            ...
            <div className="hidden lg:flex bg-black/50 backdrop-blur-md border-b border-white/50 justify-evenly items-center lg:px-24 xl:px-72">
                ...
                <Link to={'/'} className="group flex items-center">
                    <img src="../../public/Veca-Vision2.png" className="m-0 p-0 group-hover:grayscale group-hover:ease-in-out group-hover:duration-300" width={75} height={75} alt="Veca Vision Media Logo" />
                    <span className="group-hover:text-vvm-lightblue group-hover:ease-in-out group-hover:duration-300">Veca Vision Media</span>
                </Link>

                <NavLink to={'/services'} className={({ isActive }) => isActive ? 'link text-vvm-teal hover:text-vvm-teal after:bg-vvm-teal' : 'link hover:text-vvm-teal after:bg-vvm-teal'}>Services</NavLink>
            ...
            <div className={`lg:hidden w-screen float-right bg-black ${ isOpen ? 'flex flex-col' : 'hidden' }`}>
                ...
                <NavLink to={'/portfolio'} className={({ isActive }) => isActive ? 'mobile-link text-vvm-lightblue border-vvm-lightblue' : 'mobile-link border-vvm-lightblue'} onClick={() => setIsOpen(!isOpen)}>Portfolio</NavLink>
                <NavLink to={'/about'} className={({ isActive }) => isActive ? 'mobile-link text-vvm-blue border-vvm-blue' : 'mobile-link border-vvm-blue'} onClick={() => setIsOpen(!isOpen)}>About</NavLink>
                <NavLink to={'/contact'} className={({ isActive }) => isActive ? 'mobile-link text-vvm-pink border-vvm-pink' : 'mobile-link border-vvm-pink'} onClick={() => setIsOpen(!isOpen)}>Contact</NavLink>
            </div>
        </div>
    )
}
